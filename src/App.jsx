  ...
  const SafetyScreen = () => {
    ...
    return (
      <>
        ...
        <div className="safety-scroll" style={{ whiteSpace: "pre-wrap", fontSize: 12, lineHeight: 1.75 }}>
{`安全健康工作安排
《升降機工程公司｜安全培訓及承諾書》
...
PPE 包括：安全帽、防滑鋼頭安全鞋、高可視背心、防割/電工/化學手套、護目鏡、聽力保護
上鎖／掛牌（LOTO）程序：隔離電源 → 上鎖 → 掛牌 → 雙人覆核
• 安全電壓（110V / 55V）及 RCD 使用要求
• 帶電與非帶電工作的限制（非註冊電工不可帶電工作）

2）升降機／自動梯專項
機房安全：機房出入登記、通風、照明、走道暢通
井道及底坑安全：井道進入許可、通訊及通風、底坑積水處理
轎頂作業三大必須：可靠站位、通訊、Fall Protection
• 維修模式（INS / MAINT）啟動確認
• 禁止帶電作業、禁止在轎頂奔跑或跨越
升降機關鍵部件：導軌、鋼纜、曳引輪、門鎖、安全回路
自動梯作業：梳齒板、踏級、驅動鏈檢查、維修速度操作
緊急程序：Emergency Stop、釋放乘客、手動盤車、現場封鎖
...
如對程序有疑問或現場情況變更，會即時通報主管。`}
  ...
  const GpsScreen = () => {
    ...
    const SITE_LIST = hasDispatch ? assignedSites : [];
          ...
          {screen === "home" && (
            <div className="greet-bar" style={{ margin: "14px -20px -16px", padding: "14px 20px 16px" }}>
              ...
              <div className="greet-text">
                ...
                <div className="greet-sub">{EMPLOYEE.role} · 俊輝電梯工程</div>
              </div>
